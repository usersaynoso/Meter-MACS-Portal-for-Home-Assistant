...
def test_extract_meter_id_from_unique_id_handles_all_entity_suffixes() -> None:
    ...
    assert (
        extract_meter_id_from_unique_id(
            "entry123",
            "entry123_CRT_WM_3378_energy_imported_from_grid",
        )
        == "CRT_WM_3378"
    )
    assert (
    ...
    assert (
        extract_meter_id_from_unique_id("entry123", "entry123_CRT_WM_3378_safety_tripped")
        == "CRT_WM_3378"
    )
...
def test_socket_state_helpers_distinguish_connected_from_powered_on() -> None:
    ...
    assert socket_is_powered_on(None, "current") is False
